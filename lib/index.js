...
const pageFooter = module.exports = {
		...
		'page:before': function (page) {
			...
			if (configOption) {
				...
				for (var item in defaultOption) {
					...
					if (item === 'copyright') {
						defaultOption[item] += ' all right reserved, powered by <a href="https://github.com/jcsh4326/gitbook-plugin-gitlab-footer" target="_blank">Jincheng</a>';
					}
				}
			}
			...
			const issues = defaultOption.isShowIssues === true ? '\n{{ "' + defaultOption.repo + '" | listRepo("' + (process.env['GITHUB_TOKEN'] || defaultOption.token) + '", "' + defaultOption.format + '", ' + defaultOption.utcOffset + ', ' + defaultOption.issueNum + ', "' + defaultOption.git + '", "' + defaultOption.pid + '", "' + defaultOption.api + '",'+defaultOption.comments+') }}\n' : '';

			defaultOption.style = (defaultOption.style == 'normal' || defaultOption.style == 'symmetrical') ? defaultOption.style : 'normal';

			const pv = '<p><script async src="//dn-lbstatics.qbox.me/busuanzi/2.3/busuanzi.pure.mini.js"></script>' +
	...
	filters: {
		...
		listRepo: function (d, token, format, utc, issueNum, git, pid, api, showComments) {
			var content = '';
			...
			content += '<span class="issue-line"><p class="issue-header"><strong>' + ( value.length || 0 ) + '</strong> issues reported</p></span>';

			const comments =
				'<div class="hcc" el="FormHc">' +
				'<div class="hcc hc__form">' +
				'<div class="hcc hc__reply__photo" el="HCAvatar">' +
				'<img el="Avatar" class="hcc" src="$avatar$" title="">' +
				'</div>' +
				'<div class="hcc hc__reply__box">                ' +
				'<textarea el="HcText" class="hcc hc__reply__box__textarea" placeholder="Enter text..." active="true"></textarea>                ' +
				'</div>            ' +
				'<div class="hcc hc__reply__buttons" el="FButtons" style="display: block;">' +
				'<div class="hc__loader" id="hc_loader">' +
				'<svg class="hc__loader__circular">' +
				'<circle class="hc__loader__path" cx="10" cy="10" r="8" fill="none" stroke-width="2" stroke-miterlimit="5"></circle>' +
				'</svg>' +
				'</div>' +
				'<div class="hcc hc__form__send" el="HcSend">Add comment</div>' +
				'</div>' +
				'<div class="hcc hc__clear"></div>' +
				'</div>' +
				'</div>';

			content += showComments ? comments : '';
		...
		newIssue: function (token, git, pid, api, title, desc) {
			issues(git, token, api).withProject(pid).new(title, desc);
