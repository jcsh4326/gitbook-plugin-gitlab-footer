...
const issues = module.exports = {
    issues: function (host, token, api) {
        ...
        that.new = function (title, desc) {
            if(that.url) {
                that.with('title', title);
                that.with('description', desc);                
                // var param = {
                //     title: title,
                //     description: desc
                // }
                var result = syncReq('POST', getUrl());

                if(result.statusCode === 201 || result.statusCode === '201' ) {
                    if( result.getBody().toString().indexOf('"project_id":') > 1 ){
                        return true;
                    }
                }                
            }
